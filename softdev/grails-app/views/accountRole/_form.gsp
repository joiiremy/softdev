...
<div class="fieldcontain ${hasErrors(bean: accountRoleInstance, field: 'role', 'error')} required">
	...
	<g:select id="role" name="role.id" from="${com.swd.Role.list()}" optionKey="id" required="" value="${accountRoleInstance?.role?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: accountRoleInstance, field: 'account', 'error')} required">
	...
	<g:select id="account" name="account.id" from="${com.swd.Account.list()}" optionKey="id" required="" value="${accountRoleInstance?.account?.id}" class="many-to-one"/>
